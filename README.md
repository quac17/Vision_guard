...
*Luồng: (1) Offline — PC Server trích xuất embedding, xuất `face_embeddings.json` và copy sang Edge. (2) Trên xe — Camera chụp → tiền xử lý + TFLite → so khớp với DB local → gửi mã học sinh + trạng thái lên Backend. (3) Backend lưu lịch sử; Admin/Phụ huynh xem qua Frontend.*
